...
from ..risk_control.drawdown_controller import (
    DrawdownController, MarketState, PortfolioState, ControlSignal
)
...
class PortfolioConfig:
    ...
    # 回撤控制配置
    enable_drawdown_control: bool = False   # 启用回撤控制
    drawdown_control_config: Optional['DrawdownControlConfig'] = None  # 回撤控制配置
...
class PortfolioEnvironment(gym.Env):
    ...
    def __init__(self,
        ...
        # 初始化回撤控制器和奖励优化器
        if config.enable_drawdown_control and config.drawdown_control_config:
            self.drawdown_controller = DrawdownController(config.drawdown_control_config)
            self.reward_optimizer = RewardOptimizer(config.drawdown_control_config.get_reward_config())
            self.drawdown_control_signals = []
            logger.info("回撤控制器已启用")
        else:
            self.drawdown_controller = None
            self.reward_optimizer = None
            self.drawdown_control_signals = []
            logger.info("回撤控制器已禁用")
    ...
    def _initialize_state_variables(self):
        ...
        # 控制信号日志优化：跟踪上次应用的参数，避免重复日志
        self.last_applied_risk_reduction = None
        self.last_applied_max_position = None
        self.last_applied_risk_budget = None
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, Dict[str, Any]]:
        ...
        # 应用回撤控制约束
        target_weights = self._apply_drawdown_constraints(target_weights)
        ...
        # 更新回撤控制状态
        self._update_drawdown_state(portfolio_return, risk_adjusted_weights)

        # 计算增强的回撤控制奖励
        reward = self._calculate_enhanced_reward(portfolio_return, transaction_cost, risk_adjusted_weights)
    ...
    def _apply_drawdown_constraints(self, target_weights: np.ndarray) -> np.ndarray:
        """应用回撤控制约束"""
        if not self.drawdown_controller:
            return target_weights

        try:
            # 创建市场状态
            market_state = MarketState(
                prices={self.config.stock_pool[i]: self.current_prices[i]
                       for i in range(self.n_stocks)} if self.current_prices is not None else {},
                volumes={self.config.stock_pool[i]: 1000000 for i in range(self.n_stocks)},  # 简化处理
                timestamp=datetime.now()
            )

            # 创建投资组合状态
            portfolio_state = PortfolioState(
                positions={self.config.stock_pool[i]: target_weights[i]
                          for i in range(self.n_stocks)},
            ...
            # 获取回撤控制信号
            control_signals = self.drawdown_controller.execute_control_step(market_state, portfolio_state)
            self.drawdown_control_signals = control_signals

            # 应用控制信号约束
            constrained_weights = target_weights.copy()

            for signal in control_signals:
                if signal.signal_type.value == 'stop_loss':
                    # 止损信号：减少风险仓位
                    risk_reduction = signal.content.get('risk_reduction_factor', 0.5)
                    constrained_weights = constrained_weights * risk_reduction

                    # 只在风险减少因子发生变化或首次应用时记录日志
                    if self.last_applied_risk_reduction != risk_reduction:
                        logger.debug(f"应用止损约束，风险减少因子: {risk_reduction}")
                        self.last_applied_risk_reduction = risk_reduction

                elif signal.signal_type.value == 'position_adjustment':
                    # 仓位调整信号
                    max_position = signal.content.get('max_position_size', self.config.max_position_size)
                    constrained_weights = np.minimum(constrained_weights, max_position)

                    # 只在最大仓位发生变化或首次应用时记录日志
                    if self.last_applied_max_position != max_position:
                        logger.info(f"应用仓位调整约束，最大仓位: {max_position}")
                        self.last_applied_max_position = max_position

                elif signal.signal_type.value == 'risk_budget_change':
                    # 风险预算变更信号
                    risk_budget = signal.content.get('new_budget', 1.0)
                    constrained_weights = constrained_weights * risk_budget

                    # 只在风险预算发生变化或首次应用时记录日志
                    if self.last_applied_risk_budget != risk_budget:
                        logger.info(f"应用风险预算约束，新预算: {risk_budget}")
                        self.last_applied_risk_budget = risk_budget
            ...
            if total_weight > 0:
                ...
                # 如果所有权重都为0，使用等权重
                constrained_weights = np.ones(self.n_stocks) / self.n_stocks

            return constrained_weights

        except Exception as e:
            logger.error(f"应用回撤控制约束失败: {e}")
            raise RuntimeError(f"回撤控制约束应用失败: {e}") from e

    def _update_drawdown_state(self, portfolio_return: float, weights: np.ndarray):
        """更新回撤控制状态"""
        if not self.drawdown_controller:
            return
        ...
        try:
            ...
            market_data = {
                'prices': {self.config.stock_pool[i]: self.current_prices[i]
                          for i in range(self.n_stocks)} if self.current_prices is not None else {},
                'volumes': {self.config.stock_pool[i]: 1000000 for i in range(self.n_stocks)},
                'indicators': {
            ...
            # 更新投资组合状态
            portfolio_data = {
                'positions': {self.config.stock_pool[i]: weights[i] for i in range(self.n_stocks)},
                'portfolio_value': self.total_value,
                'cash': self.cash,
                'unrealized_pnl': 0.0,  # 简化处理
                'realized_pnl': 0.0
            ...
            # 通过控制器更新状态
            self.drawdown_controller.update_market_data(market_data)
            self.drawdown_controller.update_portfolio_state(portfolio_data)

        except Exception as e:
            logger.error(f"更新回撤控制状态失败: {e}")
            raise RuntimeError(f"回撤状态更新失败: {e}") from e

    def _calculate_enhanced_reward(self, portfolio_return: float,
                                 transaction_cost: float, weights: np.ndarray) -> float:
        """计算增强的回撤控制奖励"""
        try:
            # 如果没有启用回撤控制，使用原始奖励函数
            if not self.reward_optimizer:
                return self._calculate_reward(portfolio_return, transaction_cost, weights)

            # 计算基础奖励（原始奖励函数）
            base_reward = self._calculate_reward(portfolio_return, transaction_cost, weights)
            ...
            risk_adjusted_reward = self.reward_optimizer.calculate_risk_adjusted_reward(
                portfolio_return,
                current_drawdown,
                {self.config.stock_pool[i]: weights[i] for i in range(self.n_stocks)}
            )

            # 综合基础奖励和风险调整奖励
            # 使用加权平均，逐步从基础奖励过渡到风险调整奖励
            learning_progress = min(self.current_step / 1000.0, 1.0)  # 1000步后完全使用风险调整奖励
            enhanced_reward = (1 - learning_progress) * base_reward + learning_progress * risk_adjusted_reward

            # 确保奖励不是NaN或无穷值
            if np.isnan(enhanced_reward) or np.isinf(enhanced_reward):
                logger.warning(f"增强奖励异常: {enhanced_reward}，回退到基础奖励: {base_reward}")
                enhanced_reward = base_reward

            # 调试日志（降低频率）
            if self.current_step % 200 == 0:
                logger.debug(f"Step {self.current_step}: base_reward={base_reward:.4f}, "
                           f"risk_adjusted_reward={risk_adjusted_reward:.4f}, "
                           f"enhanced_reward={enhanced_reward:.4f}, "
                           f"learning_progress={learning_progress:.3f}")

            return float(enhanced_reward)

        except Exception as e:
            logger.error(f"计算增强奖励失败: {e}")
            # 发生错误时，回退到基础奖励函数，不掩盖错误
            base_reward = self._calculate_reward(portfolio_return, transaction_cost, weights)
            logger.warning(f"回退到基础奖励: {base_reward}")
            return base_reward
    ...
    def get_drawdown_metrics(self) -> Dict[str, Any]:
        ...
        if not self.drawdown_controller:
            return {
                'current_drawdown': self._calculate_current_drawdown(),
                'max_drawdown': self._calculate_current_drawdown(),  # 简化处理
                'drawdown_enabled': False
            }

        try:
            risk_metrics = self.drawdown_controller.get_risk_metrics()
            return {
                **risk_metrics,
                'drawdown_enabled': True,
                'control_signals_count': len(self.drawdown_control_signals)
            }
        except Exception as e:
            logger.error(f"获取回撤指标失败: {e}")
            raise RuntimeError(f"回撤指标获取失败: {e}") from e
    ...
    def get_risk_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"风险指标获取失败: {e}") from e